# quagga_router_linux
quagga路由器软件在linux的安装设置（ubuntu,CentOS,Debian,RedHat）
...
Ubuntu 14.04/16.04 安装配置quagga路由器软件，机器硬件配置要求，以虚拟机为例，1 vCPU，1GB RAM，4GB硬盘空间，安装一个unbuntu 16服务器版映像足够了，毕竟我们是运行在一个路由表比较小的环境，不是那种动不动处理700万条路由表的骨干路由器。   
